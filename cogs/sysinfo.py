...
class SysInfo(commands.Cog):
    ...
    async def cog_after_invoke(self, ctx):
        if not ctx.command_failed:
            if str(ctx.command) not in self.bot.command_count: self.bot.command_count[str(ctx.command)] = 1
            else: self.bot.command_count[str(ctx.command)] += 1

    @commands.group(pass_context=True, aliases=['sys'], case_insensitive=True)
    ...
    @sysinfo.command(pass_context=True, aliases=['mem'])
    ...
    async def _say(self, ctx, msg, escape=True, wait=True):
        ...
        buf = ""
        def check(msg): return msg.author == ctx.message.author and msg.channel == ctx.message.channel
        message = None
        for line in msg.splitlines():
            if len(buf) + len(line) >= 1900:
                if message: await message.edit(content=template.format(buf))
                else: message = await ctx.send(template.format(buf))
                buf = ""
                if wait:
                    try: answer = await self.bot.wait_for('message', timeout=60, check=check)
                    except asyncio.TimeoutError: return await ctx.send('Command output stopped!')
        ...
        if buf:
            if message: await message.edit(content=template.format(buf))
            else: await ctx.send(template.format(buf))
