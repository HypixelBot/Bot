...
from cogs.utils.jnbt           import tag
from cogs.utils.jnbt.shared    import scandir, read as _r, readUnsignedByte as _rub, readUnsignedInt as _rui, readUnsignedInts as _ruis
from cogs.utils.jnbt.mc.data   import _blockIDtoName
...
#A chunk has 16 sections (overall 16x256x16 blocksJSON).
#Each section has 16x16x16 blocksJSON.
...
#Region files are divided into 4KiB blocksJSON called sectors.
...
class _BaseWorld:
    ...
    def getBlockName( self, id ):
        ...
        if bIDtoN is None:
            ...
            #If this is a modded world, we can get names for blocksJSON and items from FML.ItemData
            ...
            if tag is None:
                return None
            for entry in tag:
                #Key is the internal name of the block/item, prepended with "\x01" for blocksJSON and "\x02" for items
                key = str( entry["K"] )
...
class _BaseBlock:
    ...
    * meta is the metadata value of the block. For blocksJSON with variations like wool, metadata indicates the particular variation. For all other blocksJSON, this is usually 0.
    * blockLight and skyLight indicate how much light from light-emitting blocksJSON and the sky (respectively) are reaching this block space. Both are ints in the range [0, 15].
