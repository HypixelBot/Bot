...
class Context(commands.Context):
    ...
    async def prompt(self, message, *, timeout=60.0, delete_after=True, reacquire=True, author_id=None):
        ...
        for emoji in ('\N{WHITE HEAVY CHECK MARK}', '\N{CROSS MARK}'): await msg.add_reaction(emoji)
    ...
    def tick(self, opt, label=None):
        lookup = {
            True: '<:tickYes:315009125694177281>',
            False: '<:tickNo:315009174163685377>',
        }
        emoji = lookup.get(opt, '<:tickNo:315009174163685377>')
